...
### Fixed
- The crop editor's previews and source decoding were blocked on stores whose admin Content Security Policy does not
  allow `blob:` images. The module now whitelists `blob:` for `img-src` in the admin area
  (`etc/adminhtml/csp_whitelist.xml`) and declares `Magento_Csp`.
